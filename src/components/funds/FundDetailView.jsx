...
export default function FundDetailView({ fund, onBack, onUpdateFund, defaultTab = "info" }) {
  ...
  // Mock transaction history cho chart - CHỈ TĂNG (không rút)
  const generateTransactionHistory = () => {
    const current = fund.current || 0;
    const hasAutoDeposit = fund.autoDepositEnabled;
    
    // TH1: Chỉ nạp thủ công (không đều, nhảy vọt)
    if (!hasAutoDeposit) {
      return [
        { date: '01/12', amount: 0, type: 'initial' },
        { date: '05/12', amount: current * 0.2, type: 'manual' },
        { date: '12/12', amount: current * 0.35, type: 'manual' },
        { date: '18/12', amount: current * 0.55, type: 'manual' },
        { date: '23/12', amount: current * 0.75, type: 'manual' },
        { date: '27/12', amount: current, type: 'manual' },
      ];
    }
    
    // TH2: Chỉ tự động (đều đặn, smooth)
    if (hasAutoDeposit && fund.frequency) {
      const points = 8;
      return Array.from({ length: points }, (_, i) => ({
        date: `${String(1 + i * 3).padStart(2, '0')}/12`,
        amount: (current / (points - 1)) * i,
        type: i === 0 ? 'initial' : 'auto'
      }));
    }
    
    // TH3 & TH4: Vừa thủ công vừa tự động (mixed pattern)
    return [
      { date: '01/12', amount: 0, type: 'initial' },
      { date: '03/12', amount: current * 0.12, type: 'manual' },      // Thủ công
      { date: '06/12', amount: current * 0.25, type: 'auto' },        // Tự động
      { date: '09/12', amount: current * 0.35, type: 'manual' },      // Thủ công
      { date: '12/12', amount: current * 0.47, type: 'auto' },        // Tự động
      { date: '15/12', amount: current * 0.60, type: 'auto' },        // Tự động
      { date: '18/12', amount: current * 0.72, type: 'auto' },        // Tự động
      { date: '21/12', amount: current * 0.85, type: 'manual' },      // Thủ công
      { date: '24/12', amount: current * 0.93, type: 'auto' },        // Tự động
      { date: '27/12', amount: current, type: 'manual' },             // Thủ công
    ];
  };

  const transactionHistory = generateTransactionHistory();
  const maxAmount = Math.max(fund.target || 0, fund.current || 1, ...transactionHistory.map(t => t.amount));

  // Mock transaction history - Điều chỉnh theo fund
  const mockTransactionHistory = (() => {
    const histories = [];
    
    // Fund có auto-deposit enabled
    if (fund.autoDepositEnabled && fund.amountPerPeriod) {
      const amount = fund.amountPerPeriod;
      
      // Success & failed transactions mix
      histories.push(
        {
          id: 1,
          ...
          amount: amount,
          status: 'success',
          message: 'Nạp tự động thành công theo lịch'
        },
        {
          id: 2,
        ...
        },
        {
          ...
          amount: amount,
          status: 'failed',
          message: 'Số dư ví nguồn không đủ',
          walletBalance: amount * 0.5
        },
        {
          id: 4,
      ...
      // Fund không có auto-deposit (nạp thủ công)
      const baseAmount = fund.amountPerPeriod || 1000000;
      ...
      histories.push(
        {
          ...
          status: 'success',
          message: 'Nạp thủ công thành công'
        },
        {
          id: 2,

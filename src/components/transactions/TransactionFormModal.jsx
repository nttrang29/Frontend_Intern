...
function SelectInput({
  ...
  const remainingBalanceAfterEdit = form.type === "expense"
    ...
    : walletBalance - originalIncomeAmount + amountNum;
  const showProjectedBalance = selectedWallet && (
    (form.type === "expense" && (mode !== "edit" || amountNum !== originalExpenseAmount)) ||
  ...
  const showSourceBalanceWarning = showTransferProjectedBalance && projectedSourceBalance < 0;
    ...
    const isExpenseAmountValid = form.type === "expense" 
      ? (amountNum > 0 && amountNum <= effectiveExpenseBalance)
      : (amountNum > 0);
    ...
    const handleSubmit = (e) => {
      ...
      // Validation: Kiểm tra số tiền cho loại chi tiêu/chuyển tiền
      if (!isAmountValid) {
        return;
      }
                ...
                {variant === "external" ? (
                  <>
                    ...
                    <div className="row g-3">
                      ...
                      {/* Hiển thị số dư hiện tại và số dư dự kiến sau khi lưu (chi tiêu) */}
                    {selectedWallet && (
                      <>
                        ...
                        {showProjectedBalance && (
                          <div className="form-text">
                            {"Số dư dự kiến:"}{" "}
                            <strong className={remainingBalanceAfterEdit < 0 ? "text-danger" : ""}>
                              {formatMoney(Math.max(remainingBalanceAfterEdit, 0), selectedWallet.currency || "VND")}
                            </strong>
                          </div>
                        )}
                        {showBalanceWarning && (
                          <div className="text-danger small">
                            {"Số dư sẽ âm sau giao dịch này."}
                          </div>
                        )}
                      </>
                    )}
                    {/* Ẩn thông báo lỗi chi tiêu, chỉ cảnh báo số dư âm; vẫn giữ cho loại khác nếu cần */}
                    {showAmountError && form.type !== "expense" && (
